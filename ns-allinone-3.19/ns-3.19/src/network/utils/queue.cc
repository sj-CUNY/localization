...
Queue::Queue() : 
  ...
  m_nTotalDroppedPackets (0), m_delay (0), m_calc (0)
{
...
Queue::Enqueue (Ptr<Packet> p)
{
  ...
  tTag.SetTimeValue( Simulator::Now ());
  ...
  if (packet != 0)
    {
      ...
      m_nPackets--;
      TimeTag  tTag;
      if (packet->RemovePacketTag(tTag) == true)
      {
	      Time tx = tTag.GetTimeValue();
	      if (m_delay != 0)
...
void 
Queue::SetDelayTracker(Ptr<TimeMinMaxAvgTotalCalculator> delay)
{
...
void 
Queue::SetCounter (Ptr<CounterCalculator<> > calc)
{  
     m_calc = calc;
}
...
TimeTag::GetTypeId (void)
{
  static TypeId tid = TypeId ("TimeTag")
    ...
    .AddAttribute ("TimeTag",
                   "Time stamped on a packet",
                   EmptyAttributeValue (),
                   MakeTimeAccessor (&TimeTag::GetTimeValue),
                   MakeTimeChecker ())
  ;
  return tid;
}
...
{
  int64_t t = m_timeValue.GetNanoSeconds();
...
void 
TimeTag::SetTimeValue (Time value)
{
  m_timeValue = value;
}
Time 
TimeTag::GetTimeValue (void) const
{
  return m_timeValue;
}
} // namespace ns3
