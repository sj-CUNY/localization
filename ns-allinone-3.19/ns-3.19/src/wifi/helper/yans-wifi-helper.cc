...
)
{
  ...
  *stream->GetStream () << "r " << Simulator::Now ().GetSeconds () << " " << context << " " << *p << " " << snr << " " << mode << std::endl;
}
static void
  ...
  bool explicitFilename)
{
  ...
  oss.str ("");
  oss << "/NodeList/" << nodeid << "/DeviceList/" << deviceid << "/$ns3::WifiNetDevice/Channel/YansRSS";
  Config::Connect (oss.str (), MakeBoundCallback (&AsciiChannelRssWithContext, stream));
